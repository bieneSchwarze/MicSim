...
The example data on migration between Spain, Sweden and the Netherlands has been prepared by Claudio Bosco (European Commission), Daniela Ghio (European Commission), Maurizio Teobaldelli (European Commission) and Sabine Zinn (German Socio-Oeconomic Panel, Humboldt University Berlin). As data source EUROSTAT data has been used. By purpose the example has been designed to be small. MicSim can also handle a larger size of cases but to be efficient concerning rumn times this requires a bit more computational power with more CPUs. 
...
### Rates Definition
Beware: Rates have to be given at least for age [0,maxAge-1] and for all years within the simulation horizon. 
```{r}
# Load rates from external data (one column for one transition)
rates <- migrExpRates
...
For running this you have to remove the comment signs in the beginning of each line. This example uses three cores.
Give as many seeds as you use cores. That way you can replicate your results.
```{r}
